...
int main(int argc, char** argv)
{
  ...
  try{
    ...
    for(int i= 0 ; i < n_dfem_p ; i++)
    {
      ...
      {
        if(fabs( (expected_dimensionless_mass[i][j] - matrix(i,j) )/matrix(i,j)) > tol )
        {
    ...
    for(int i= 0 ; i < n_dfem_p ; i++)
    {
      for(int j=0; j < n_dfem_p ; j++)
      {
        if(fabs( (expected_L_mu_positive[i][j] - matrix(i,j) )/matrix(i,j)) > tol )
        {
          std::stringstream err;
    ...
    for(int i= 0 ; i < n_dfem_p ; i++)
    {
      for(int j=0; j < n_dfem_p ; j++)
      {
        if(fabs( (expected_L_mu_negative[i][j] - matrix(i,j) )/matrix(i,j)) > tol )
        {
          std::stringstream err;
          err << "Discrepancy calculating L for mu < 0 in row: " << i << " column: " << j <<std::endl;
          throw Dark_Arts_Exception(FEM, err.str() );
    ...
    for(int i= 0 ; i < n_dfem_p ; i++)
    {
      if(fabs( (expected_f_mu_negative[i] - vector(i) )/vector(i)) > tol )
      {
        std::stringstream err;
    ...
    for(int i= 0 ; i < n_dfem_p ; i++)
    {
      if(fabs( (expected_f_mu_positive[i] - vector(i) )/vector(i)) > tol )
      {
        std::stringstream err;
        err << "Discrepancy calculating f for mu > 0 in row: " << i << std::endl;
        throw Dark_Arts_Exception(FEM, err.str() );
