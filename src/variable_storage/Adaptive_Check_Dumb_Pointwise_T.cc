...
Adaptive_Check_Dumb_Pointwise_T::Adaptive_Check_Dumb_Pointwise_T(const Temperature_Data& t_old , const K_Temperature& k_t ,  const Time_Data& time_data,
...
bool Adaptive_Check_Dumb_Pointwise_T::adaptive_check(const double dt, double& adapt_criteria) 
{ 
  adapt_criteria = 0.;
  ...
  for(int c = 0 ; c < m_n_cells ; c++)
  {
    ...
    {
      double err = fabs( sum_k_t(el) )/( 2.*m_t_old_vec(el) + sum_k_t(el)+ m_denom_min) ;
