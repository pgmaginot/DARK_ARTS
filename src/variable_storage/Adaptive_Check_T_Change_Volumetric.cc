...
{ 
  ...
  for(int grp = 0 ; grp < m_n_groupings ; grp++)
  {
    ...
    for(int c = 0 ; c < m_n_cell_per_grouping ; c++)
    {
      ...
      m_fem_quadrature.evaluate_variable_at_quadrature_pts(m_t_new_vec , m_dfem_at_qp , m_t_old_evals);
      
      double dx = m_cell_data.get_cell_width(cell);
      for(int q = 0 ; q < m_n_qp ; q++)
      {
        delta_cell += m_qp_w_at_dfem[q]*( m_t_new_evals[q] - m_t_old_evals[q])*( m_t_new_vec[q] - m_t_old_evals[q])  ;
